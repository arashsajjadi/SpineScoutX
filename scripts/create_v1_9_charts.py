...
def chart_timeline() -> None:
    ...
    for i, (ver, label, color, outcome) in enumerate(versions):
        ...
        # Version label (below)
        ax.text(
            i, -0.32, ver, ha="center", va="top", fontsize=10.5, fontweight="bold", color=C_DARK
        )
...
def chart_outcome_matrix() -> None:
    ...
    for i, row in enumerate(MATRIX):
        for j, val in enumerate(row):
            bg = COLOR_MAP[val]
            rect = plt.Rectangle([j - 0.44, i - 0.42], 0.88, 0.84, color=bg, zorder=2, linewidth=0)
            ax.add_patch(rect)
            ax.text(
                j,
                i,
...
def chart_triage_recall() -> None:
    ...
    ax.plot(
        budgets, raw, "o--", color=C_GRAY, lw=2.5, markersize=7, label="Raw argmax only (0.724)"
    )
    ax.plot(
        budgets,
        triage,
    ...
    ax.annotate(
        ...
        xy=(15, 0.933),
        xytext=(16.5, 0.80),
        arrowprops={"arrowstyle": "->", "color": C_BLUE, "lw": 1.5},
        fontsize=11,
        color=C_BLUE,
        fontweight="bold",
    )
    ...
    ax.set_title(
        "v1.7 Triage — Effective Severe Recall vs Review Budget\n"
        "(review/triage metric — NOT raw argmax grading)",
        pad=10,
    )
    ax.legend(loc="lower right", fontsize=11)
    ax.text(
        0.02,
        0.03,
        "Research-only · not diagnostic · deployed grader unchanged",
        transform=ax.transAxes,
        fontsize=9,
        color="#999999",
    )
...
def chart_what_we_learned() -> None:
    ...
    ax.text(
        0.5,
        0.97,
        "SpineScoutX — What We Learned (v1.0 – v1.8c)",
        ha="center",
        va="top",
        fontsize=16,
        fontweight="bold",
